...
def _Fast_MatMul_grad(op, grad):
    ...
    grad_a = fast_mm_module.FastMatMul(a_matrix=grad, b_matrix=bt, epsilon=epsilon_values.get(mm_algo, 1e-2), steps=1, numthreads=num_threads)
    grad_b = fast_mm_module.FastMatMul(a_matrix=at, b_matrix=grad, epsilon=epsilon_values.get(mm_algo, 1e-2), steps=1, numthreads=num_threads)
...
if __name__ == '__main__':
    ...
    with open(f'ACCURACY_TEST_{mm_algo}_layers{num_layers}_nodes{nodes}_epochs{EPOCHS}_bs{batch_size}_accuracy_and_loss.txt', 'wt') as file:
