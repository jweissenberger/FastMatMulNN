...
\begin{enumerate}
	...
	\item \RZ{present performance improvements of VGG-19 network of 15\% over the classical matrix multiplication.}
\end{enumerate}
...
\label{fig:vgg_par6}}
%
\caption{Total time test on the fully connected layes on VGG-19 using $\dims{4,4,2}$ and dgemm}
\label{fig:vgg}
\end{figure*}


\subsection{VGG-19 Network}
VGG19 is a convolutional neural network typically trained on the imagenet dataset as a classification task. The model is one of the highest performing algorithms for this task. 
The network is composed of 19 layers, 16 convolutional and 3 fully connected. 
The 3 fully connected layers consist of 25,088, 4096 and 1000 nodes and are placed at the end of the network. 
These create very large multiplications in forwards and backwards propagation. 
Using faster APA algorithms for the multiplication we are able to speed up the training of the network. 
...
Replacing the classic matrix multiplication algorithm in these layers with the Smirnov 442 algorithm provides up to a 10\% speed up of the fully connected layers during training with 6 threads and a 15\% speed up using a single thread. 
\cref{fig:vgg} plot the training time of each network with increasing batch sizes 
