...
Our performance results on a large MLP network show overall multithreaded performance improvements of up to \GB{W\%}.
\end{abstract}
...
%
\caption{Multi-Layer Perceptron network accuracy for MNIST data set}
\label{fig:NN_acc}
\end{figure*}

To measure the effect of the matrix multiplication error introduced from the APA algorithms on neural network accuracy, we trained a multi-layer perceptron (MLP) network on the MNIST dataset \cite{LBBH98} and measured its accuracy over the course of training. 
The network was created using Tensorflow version 2.2.0.
...
We see more variability in the test error due to the sensitivity inherent in generalizability of the model, though all algorithms achieve between 97\% and 99\% test accuracy.

\section{Neural Network Performance}
\label{sec:NNperf}

The neural networks used for performance benchmarking were 6 layer (4 hidden layers), fully connected networks and were trained on the MNIST dataset. The purpose of these experiments was to measure the speed up in training time provided by the APA algorithms and not to measure the accuracy performance of the networks. The APA algorithms were used only in the multiplications in the hidden layers during forwards and backward propagation. 

Similar to other benchmarks [https://github.com/Emma926/paradnn] on fully connected networks, we measured the performance of each algorithm with the number of nodes in each hidden layer from 2^9 to 20^13. We also matched the batch size of each network to the number of nodes in the hidden layer which produces square multiplications in the hidden layers. We ran these tests using 1, 6 and 12 threads.

The single threaded results began to show speed up over the classical algorithm when the matrix dimensions were 2^10 and were able to show over a 20\% speed up when the dimensions were 2^13. It was more difficult to get performance improvements with more threads but both 6 and 12 thread results were able to show improvement in the larger dimensions. At 6 threads, the best algorithms showed small improvements when the dimensions were 2^11 and were able to get over a 10\% improvement at 2^13. The 12 thread algorithms were not able to see an improvement until the dimensions were 2^12 and were not able to get 10\% improvement at 2^13.
