...
We demonstrate that APA algorithms can be efficiently implemented and parallelized for shared-memory platforms to obtain up to \GB{Z\%} speedups over the fastest implementation of the classical algorithm.
...
\GB{add more details on experiment here...}

\begin{enumerate}
	\item confirmed that optimal lambda values are with a factor of 4 of theoretical (for these random inputs)
	\item some errors were lower than expected (555 and 7222), we think this is because of fractions that appear in the coefficients of the largest negative powers of lambda
	\item legend is ordered according to theoretical error
	\item not much fluctuation over dimensions
\end{enumerate}

\begin{figure}
\centering
\renewcommand{\datafile}{data/accuracy.dat}
...
We note that the original code generation tool was designed primarily for exact algorithms, and we generalized it to apply to all of the APA algorithms we consider here.
...
\GB{add sequential performance results for all 12 algorithms, compared to \texttt{gemm}}
...
\GB{add parallel performance results for all 12 algorithms, compared to \texttt{gemm}}

\begin{figure}
\centering
\renewcommand{\datafile}{data/matmul_par.dat}
\renewcommand{\threads}{12}
\input{plots/matmul_perf}
\caption{Multithreaded matrix multiplication performance}
\label{fig:matmul_seq}
\end{figure}
