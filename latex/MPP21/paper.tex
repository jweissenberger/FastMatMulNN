...
Even a slight reduction in flops could save a large amount of time and money.
This steep cost has spurred a surge of research into more efficient hardware, better algorithms, and techniques for trading off accuracy for performance.
...
We can also expressed for $\hat{C}_{11}$ as $\hat C_{11} = A_{11} \cdot B_{11} + A_{12} \cdot B_{21} - \lambda (A_{12} \cdot B_{11})$, where $E = A_{12} \cdot B_{11}$ is the error matrix.
