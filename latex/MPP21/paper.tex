...
\GB{TBD...}
\begin{enumerate}
	\item define working precision $2^{-d}$
	\item define $\varphi$ and $\sigma$ and give intuition
	\item define optimal $\lambda=O(2^{-d/(\varphi+\sigma)})$
	\item define error given optimal $\lambda$: $O(2^{-d\sigma/(\varphi+\sigma)})$
\end{enumerate}
...
These speedups are typically not attained because of degradation in performance for smaller matrix dimensions and the overhead of matrix additions.
